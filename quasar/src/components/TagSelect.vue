<template>
  <q-select menu-self="bottom start" menu-anchor="top start" :label=" (parent ? $t('tag.parent') : $t('tag.many')) + $t('optional')" :multiple="!parent" use-chips use-input :counter="!parent" v-model="selected" :options="tagoptions" option-value="id" option-label="name" @filter="filterTags"/>
</template>
